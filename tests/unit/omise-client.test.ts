...
describe('OmiseClient', () => {
  ...
  describe('token API', () => {
    describe('createToken', () => {
      it('should create token successfully using vault URL and public key', async () => {
        ...
        const result = await omiseClient.createToken(tokenParams);
        ...
        expect(mockAxiosInstance.post).toHaveBeenCalledWith(
          ...
          {
            auth: {
              username: 'pkey_test_123',
              password: ''
            }
          }
        );
        expect(mockLogger.info).toHaveBeenCalledWith('Creating token', { 
          cardNumber: '************4242' 
        });
        expect(mockLogger.info).toHaveBeenCalledWith('Using vault URL for token creation', { 
          vaultUrl: 'https://vault.omise.co/tokens' 
        });
        expect(mockLogger.info).toHaveBeenCalledWith('Token created successfully', { tokenId: mockToken.id });
      });

      it('should throw error for invalid token response', async () => {
        mockAxiosInstance.post.mockResolvedValue({ data: null });

        await expect(omiseClient.createToken({
          card: {
            number: '4242424242424242',
            name: 'John Doe',
            expiration_month: 12,
        ...
        })).rejects.toThrow('Omise API Error: Invalid token response');
      });
    });

    describe('getToken', () => {
      it('should retrieve token successfully using vault URL and public key', async () => {
        const mockToken = createMockToken();
        const tokenId = 'tokn_123';

        mockAxiosInstance.get.mockResolvedValue({ data: mockToken });

        const result = await omiseClient.getToken(tokenId);

        expect(result).toEqual(mockToken);
        expect(mockAxiosInstance.get).toHaveBeenCalledWith(
          'https://vault.omise.co/tokens/tokn_123',
          {
            auth: {
              username: 'pkey_test_123',
              password: ''
        ...
        expect(mockLogger.info).toHaveBeenCalledWith('Retrieving token', { tokenId });
      });

      it('should throw error for invalid token response', async () => {
        mockAxiosInstance.get.mockResolvedValue({ data: null });

        await expect(omiseClient.getToken('tokn_123')).rejects.toThrow('Omise API Error: Invalid token response');
      });
    });
  });
